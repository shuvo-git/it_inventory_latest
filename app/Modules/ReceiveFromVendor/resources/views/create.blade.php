        ...
        <div class="panel panel-default card-view">
            ...
            <div class="panel-wrapper collapse in">
                <div class="panel-body">
                        ...
                        {{Form::open(['route'=>'send-to-repair.store','method'=>'post'])}}
                            @include('SendToRepair::_form')
                        {{Form::close()}}
...
<script>
    ...
    function getProductExpiry(event,id){
        $.ajax({
            method: "POST",
            url: "{{ url('getProductExpiryDate') }}",
            data: { product_id: id },
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}
        })
        .done(function( data ) {
            if(data) {
                console.log(data);
                $(event.target).parent().next().children(0).val(data)  ;
                //$('#stockin_details_id').empty().append(data);
            }
        });
    }
